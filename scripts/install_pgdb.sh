...
echo "Installing postgres database version 12"
...
if [[ "$OS" == "rhel" ]]; then
   ...
   apt-get -y install postgresql  &>> $log_file
fi
...
if [[ "$OS" == "rhel" ]]; then    
    ...
    sudo -u postgres /usr/lib/postgresql/12/bin/pg_ctl initdb -D $PGDATA &>> $log_file
fi
...
pg_systemd=/lib/systemd/system/postgresql.service
fi
rm -rf $pg_systemd
...
then
echo "ExecStart=/usr/lib/postgresql/12/bin/pg_ctl start -D ${PGDATA} -l ${PGDATA}/pg_log" >> $pg_systemd
echo "ExecStop=/usr/lib/postgresql/12/bin/pg_ctl stop -D ${PGDATA}" >> $pg_systemd
echo "ExecReload=/usr/lib/postgresql/12/bin/pg_ctl reload -D ${PGDATA}" >> $pg_systemd
fi
echo "" >> $pg_systemd
...
then
systemctl enable postgresql.service &>> $log_file
